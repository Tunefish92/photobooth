...
def test_done_returns_to_the_selected_modes_confirm_screen(running_app):
    ...
    confirm screen -- one tap from another round -- not the tile grid.
    (Retake doesn't need a corresponding fix: it goes straight back to
    GREETER for another attempt at the same session, per
    PhotoboothStateMachine.retake(), and never touches IDLE/
