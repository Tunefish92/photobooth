...
class AppController(QObject):
    ...
    def done(self) -> None:
        ...
        # that mode's confirm screen, not the tile grid, and stays there
        # across repeat rounds. The only way out is the confirm screen's
        # explicit Back button (cancelModeSelection()). A mode reached via
        # the GPIO shortcut with nothing selected (mode == "") correctly
        # falls through to the tile grid instead, since there's no confirm
        # screen it was ever shown.
        if mode:
            self._selected_mode = mode
            self.selectedModeChanged.emit()
